if defined?(ActiveRecord)
  ActiveRecord::Base.class_eval do
    # Rails 3 callback style
    if respond_to?(:after_initialize)
      after_initialize :after_initialize_with_rack_bug

      def after_initialize_with_rack_bug
        Rack::Bug::ActiveRecordPanel.record(self.class.base_class.name)
      end
      private :after_initialize_with_rack_bug
    elsif instance_methods.include?("after_initialize")
      def after_initialize_with_rack_bug
        Rack::Bug::ActiveRecordPanel.record(self.class.base_class.name)
